 ...
 */
const fetchRequest = (url, options = {}, responseType = 'json') => {
    // 预设请求和响应拦截器
 ...
 * @returns {Promise} - 返回一个promise，解析为请求的响应结果。
 */
const getRequest = (url, params = {}, options = {}) => {
    // 构造带查询字符串的URL
 ...
 * @returns {Promise} - 返回一个promise，解析为请求的响应结果。
 */
const deleteRequest = (url, params = {}, options = {}) => {
    // 构造带查询字符串的URL
    if (params && Object.keys(params).length) {
        url += (url.indexOf('?') === -1 ? '?' : '&') + toQueryString(params)
    }
 ...
 * @returns {Promise} - 返回一个promise，解析为请求的响应结果。
 */
const postRequest = (url, data = {}, json = true, options = {}) => {
 ...
 * @returns {Promise} - 返回一个promise，解析为请求的响应结果。
 */
const putRequest = (url, data = {}, json = true, options = {}) => {
    const headers = new Headers(options.headers || {})
    let bodyData
    if (json) {
        // 如果json为true，发送JSON格式的数据
        headers.append('Content-Type', 'application/json')
        bodyData = JSON.stringify(data)
    } else {
        // 如果json为false，发送FormData格式的数据
        bodyData = new FormData()
        for (const [key, value] of Object.entries(data)) {
            bodyData.append(key, value)
        }
 ...
 */
const downloadRequest = (url, data, fileName, method = 'GET') => {
